...
import { createVideo, findVideoById, findVideos } from "./video.service";
...
export async function streamVideoHandler(req: Request, res: Response) {
  ...
  if (!range)
    return res.status(StatusCodes.BAD_REQUEST).send("Range must be specified");
  ...
  const chunkEnd = Math.min(chunkStart + CHUNK_SIZE, fileSizeInBytes - 1);
